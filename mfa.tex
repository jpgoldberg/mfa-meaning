...
Authentication, very very roughly, is the process of proving one's identity to some system or service.
...
the security stems from the fact that it requires different attacks and different attacker capabilities to compromise the different types.
...
The ideal example of multiple factors is chip and PIN payment cards. Although it is not what people think about when they consider MFA it illustrates security property \prop{FS} in a distilled form.
...
(1)~We actively offered services designed for business-like organizations, and (2)~we did introduce an authentication component.
...
chose weaker account passwords\footnote{%
  ...
  In older documentation or reports of customer conversations in the past you may see it referred to as “Master Password” or “MP”.}
...
and we do not believe that we have succeeded at explaining it to any but the
...
For example, we had not been \emph{explicitly} aware of \prop{Notif} as a valuable security property until people pointed it out to us.
